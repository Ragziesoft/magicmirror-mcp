...
const haPlugin: MCPPlugin = {
  ...
  async register(server, context) {
    ...
    server.tool(
      ...
      "Call a Home Assistant service to control devices or trigger automations (e.g. turn lights on/off, lock doors, run scripts)",
      {
    ...
    server.tool(
      "homeassistant.entities.list",
      "Get the state of multiple Home Assistant entities in a single call",
      {
      ...
      async ({ entity_ids }) => {
        try {
          const results = await Promise.allSettled(
            entity_ids.map(id => client.getState(id))
          )
          const output = entity_ids.map((id, i) => {
            const r = results[i]
            return r.status === "fulfilled"
              ? { entity_id: id, ...r.value }
              : { entity_id: id, error: String((r as PromiseRejectedResult).reason) }
          })
